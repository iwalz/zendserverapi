...
class ServerInfo extends \ZendServerAPI\Mapper\Mapper
{
    ...
    public function parse($xml)
    {
        ...
        $server->setId(
                (int)$xml->responseData->serverInfo->id
        );
        $server->setName(
                (string)$xml->responseData->serverInfo->name
        );
        $server->setAddress(
                (string)$xml->responseData->serverInfo->address
        );
        $server->setStatus(
                (string)$xml->responseData->serverInfo->status
        );
        ...
        if(isset($xml->responseData->serverInfo->messageList->error))
        {
            $messageList->setError(
                (string)$xml->responseData->serverInfo->messageList->error
            );
        }
        if(isset($xml->responseData->serverInfo->messageList->info))
        {
            $messageList->setInfo(
                (string)$xml->responseData->serverInfo->messageList->info
            );
        }
